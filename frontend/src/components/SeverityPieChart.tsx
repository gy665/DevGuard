...
// 1. Define the shape of the data this component expects as a prop.
...
// 2. Define the shape of the props object.
interface PieChartProps {
  data: VulnerabilityData;
}

// 3. Define the shape of the objects in our severities array for type safety.
...
const SeverityPieChart: React.FC<PieChartProps> = ({ data }) => {
  // 4. We type our state to match the shape of our data.
  const [animatedValues, setAnimatedValues] = useState<VulnerabilityData>({ critical: 0, high: 0, medium: 0, low: 0 });
  ...
  // 5. We type our array to ensure every object inside it has the correct shape.
  ...
  return (
    <div className="bg-slate-800 p-6 rounded-lg border border-slate-700 h-full flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center">
            ...
            <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                ...
                return severities.map((severity, index) => {
                    ...
                    return (
                    <circle key={index} cx="50" cy="50" r="15.9155" fill="transparent" stroke={severity.strokeColor} strokeWidth="10" strokeDasharray={strokeDasharray} strokeDashoffset={strokeDashoffset} className="transition-all duration-300" style={{ filter: 'drop-shadow(0 0 4px rgba(0,0,0,0.3))' }}/>
                    );
                });
                })()}
            </svg>
        ...
        <div className="mt-6 grid grid-cols-2 gap-3">
            ...
            <div key={index} className="flex items-center gap-2 opacity-0 animate-slide-in" style={{ animationDelay: `${index * 100 + 300}ms`, animationFillMode: 'forwards' }}>
